...
- [x] 0.7 严格校验：`aiws change validate cleanup-batch-20260919 --strict` → 实测：`ok: change validated`（`exit=0`）；加跑默认未启的 `--check-evidence --check-scope` → 本批全部改动文件均在 allow-list 内，唯一残留为 `.aiws/memory-bank/` 两条（用户在先产物、不属本批；2026-09-20 为解开 `change finish` 的脏树拒绝，owner 裁决以独立提交 `dae7ae6` 搭在本分支入库，但不进 `### In Scope`——该列表已满 12 条上限且它们非本批范围，故越界清单不变），详见 evidence §A-13、§A-23 与 PROB-011
...
- [x] 2.17 归档前解阻（2026-09-20）：`aiws change finish` 因脏树拒绝（`Refusing to finish with a dirty working tree`，`exit=2`），挡路的是你在先 8-31 的 `.aiws/memory-bank/.index.yaml`（+11 行）与 `decision/analysis/chatwoot-offline.md`（979 B）。owner 裁决按 `review/quality-review.md` §L3 当初的建议**拆成独立 chore 提交 `dae7ae6`** 搭在本分支入库（提交前逐字读过该笔记：运维结论 + 站点 URL + `Layout.astro` 位点，无凭据），而非 stash 或丢弃。归属不变 ⇒ **不**加进 plan 的 `### In Scope`（已满 12 条上限，且它们非本批工作），`--check-scope` 因此继续报这两条（实测 `exit=2`，清单逐字未变，`86-scope-after-memorybank.log`）。连带同步 plan §Scope、本文件 0.7/3.5/3.7、`evidence/verify-before-complete.md` §A-23、`dev-log.md` §8.9、`follow-ups.md` 的 PROB-011 观察项，并追加台账第 51–52 条
...
- [x] 4.4 收尾：`aiws verify-bc`（`ok: all gates passed (tier=strict)`，末遍 `87-gates-memorybank.log` 复跑于本轮全部编辑定格后）→ `aiws commit`（本批 `c9cbc2d..dae7ae6`：`e14c684` 清理主体、`b0bff23` 审查遗留、`42ef9c2` 口令裁决、`4b9dd5e` `reference/` 出库、`735bc09` PROB-005/006 合并修复、`88c9a80` 提交后审查轮收口、`dae7ae6` memory-bank 独立入库）→ **L3 深度安全审查 0 findings** → 推送：`gitee/main` 与 `origin/main` 均 `c9cbc2d..88c9a80`（`git ls-remote --heads` 逐个核实，退出码单独取；GitHub 首两次 `443` 超时后重试可通）→ `aiws change finish cleanup-batch-20260919` 紧随本提交执行。注：`aiws change status` 的归档门禁要求本条已勾，故 finish **自身**的回显不回填到这里（与 dev-log §8.2 的自指限制同源），归档结果以命令回显为准
