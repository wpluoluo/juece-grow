...
- `.aiws/memory-bank/**`（你在先的 8-31 产物）会被持续报越界。这是工具的**正确行为**（它确实不属于本批）。2026-09-20 追加实测，把这条风险说得更硬：owner 裁决把这两条以独立提交 `dae7ae6` 入库后，`--check-scope` 的越界清单**逐字未变**（`86-scope-after-memorybank.log`，`exit=2`）⇒ 「提交掉它」并不能让门禁全绿，工具对工作树脏改动和分支 diff 一视同仁地算越界。真正能清掉的只有两条路：进 allow-list（本批已满 12 条上限）或让这些提交走别的分支。结论不变：只要范围外改动跟着本分支走，`--check-scope` 就永远不可能全绿——别把它当成"可忽略的告警"。
