...
| 14 | 机器可核验台账本身：`evidence/verification.jsonl`（生成器 `.aiws/tmp/cleanup-batch-20260919/run-verification.mjs`，补记脚本 `rerun-scope-gate.mjs`、`probe-db-after-e2e.mjs`、`run-cold-e2e.mjs`、`final-tree-rerun.mjs`、`append-postfix-records.mjs`、`append-gate-records.mjs`、`append-refdrop-records.mjs`，去重工具 `dedupe-ledger.mjs`，目录计数 `count-dirs.mjs`） | **36 条记录，全部 `status=success`**（36 行对应 **35 个不同命令**：加强门禁在 scratch 清理前后各跑过一次，#9/#20 两条工件不同、有意保留），字段按 verify-bc M5 门禁要求（`command / exit_code / status / started_at / finished_at / artifact`，负向用例另带 `expected_exit_code`）。覆盖：build / 类型 / **三条** backup 负向（`exit=1` 才是通过）/ 三组库侧探针 / `plan-verify` / `validate --stamp` / `--strict` / `--strict --check-evidence --check-scope`（`exit=2` 才是通过）/ `tasks validate` / 端口空闲预检 / 双服务就绪轮询 / 路由预热 / 全量 e2e ×3（正常轮、缓存真空轮、提交后修复轮）/ 冷启动复跑（§A-15）。逐条时间戳与工件路径一一对应，可离开本文档独立复核。**两次自我纠错都在盘**：(i) 首版台账作废——原因见 §A-13 的仓库根少一层；(ii) `final-tree-rerun.mjs` 的「整表重写式 append」把 23 行写成 47 行且已随 `e14c684` 进 git，`dedupe-ledger.mjs` 实测 `before=47 after=24 重复=0 非success或期望码不符=0` 后修脚本为「只写新行 + 回读核对 +1」（详 `dev-log.md` §8.5） | `evidence/verification.jsonl`、`.aiws/tmp/cleanup-batch-20260919/{10..64}-*` |
