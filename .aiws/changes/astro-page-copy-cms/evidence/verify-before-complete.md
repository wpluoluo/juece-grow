...
> Change: `astro-page-copy-cms` ｜ 分支 `change/astro-page-copy-cms`（全程未提交，见 §7）
> 时间：2026-09-21（本地容器库 `127.0.0.1:5434` / Docker `juece-grow-postgres`；CMS dev `127.0.0.1:3000`；公开站 dev `127.0.0.1:4321`。**线上地址未连接、未执行任何命令**）
...
- **4.3 只剩两步，且各自卡前置，不许顺手做**：① 推送须 owner 明确确认并**先过 L3 深度安全审查**；② `aiws change finish` 需 CMS 在 `:3000` 可达（404 ≠ 没起）且归档只挂在 `--push` 分支 ⇒ 本地路径为 `aiws change archive <id>` + 手工提交。**本地提交本身已完成**（`0872bd0`，未 `--amend`、未推送）。
- **4.4 已完成**：规划轮 `decision://astro-page-copy-cms/plan` + 交付轮 `decision://astro-page-copy-cms/delivery`（含 `delivery → plan` 关联与逐字读回），见 §7 行 36。memory-bank 按 owner 裁决单独一次 `chore(memory)` 提交，不混进代码提交。
- **台账仍欠本轮尾部行**（`453` 起那批，本轮新增 `490`–`499`）：下一轮 p15 appender 用同一套前置自检补登。待补清单的唯一真值与分类见下方「尾部台账记账的真实状态」，此处不复述；补登时的既有坑另写在 `follow-ups.md` §4。

尾部台账记账的真实状态（P14 之后，与 §0 第 2、3 行一致）：

- **字节层已经没有风险**，且不依赖手工习惯：`449`（全量镜像，正则修宽后复跑 `tmp 侧=428 归档侧=428 新复制=0`）+ 参数化尾部镜像 `481-mirror-tail-window.mjs <LO> <HI>`（每轮续窗口，上界小于自身编号）+ 同一批里 `cp` 封口它自己的日志。终局证据是末次那支**刻意不落 tmp 日志**的审计运行：`编号日志总数=462 台账有行=396 归档有字节=462 归档缺字节=0 判定=OK`（§7 行 34）⇒ 4.3 提交后没有任何一份日志会随 tmp 消失。此前每一支带自己日志的运行都必然报 `归档缺字节=1`（缺的就是它自己那份还没写完的日志），那是结构而非缺陷。
- **还欠台账行的只有本轮尾部**：待补清单的**唯一真值是那只从磁盘反查的审计器逐份点名的输出**（`node .aiws/tmp/astro-page-copy-cms/448-audit-logs-missing-from-ledger.mjs`，它把「有字节但台账无行」的每一份列在 stdout 里），本文不复述清单——任何写死在文档里的份数都会随下一轮运行过期。本轮（4.3/4.4）新增的是 `490`–`499` 十份：`490` 提交前门禁、`491`/`499` 交付轮记忆的两次写（`499` 是对 `491` 的口径修正）、`492` 关联与读回、`493`–`498` 文档改准后的复跑链。既有分类不变：自己造成的红是 `453`（首跑被组内 `>` 截断）与 `461`（首跑缺 no-op 判定即抛）两条；`459`（B4 点名 3.10 过期）与 `464`/`465`/`473`/`474`（唯一缺字节的那份就是审计器自己还没写完的日志）是设计红，按 `expected_exit_code=1` 记。`472` 那支脚本至今没有自己的 `.log`，正文在 `473` 里。`417`–`452` 已在册。
- **一个已知坑，别到补登时才发现**：`461` 里那次抛错的运行**没有落 `_exit=1`**（堆栈在、退出码行没有），而 p14 起的自检要求「`exit_code` 必须字面出现在工件的 `_exit=` 行里」。⇒ 补登 `461` 时要么按 `expected_exit_code=0` 记那次成功的复跑、要么把「红本设计」写进排除表，**不许凭记忆补一个 1**。教训本身：把红留在盘上时，要连同它的退出码一起留。
