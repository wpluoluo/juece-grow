...
**未完项**：4.3 本地 `aiws commit`（**推送需 owner 明确确认并先过 L3 深度安全审查**；`aiws change finish` 需 CMS 在 `:3000` 可达，404 ≠ 没起，且归档只挂在 `--push` 分支上 ⇒ 本地路径为 `aiws change archive <id>` + 手工提交）→ 4.4 `aiws memory write decision://astro-page-copy-cms/*`（规划轮 + 交付轮的 schema/接缝决策）。
...
- **还欠台账行的只有本轮尾部**：终局那次审计实测 `有字节但台账无行=31`（`447b`、`463b`、`453`–`471`、`473`、`474`、`475`–`482` 里未被排除表覆盖的那些；`472` 那支脚本没有自己的日志，正文在 `473` 里）。这个 31 是**那次运行当时**的数——此后每多一支带日志的运行就多一份，补登与续镜像都走同一套（`481` + `cp` + `448`），不必回来改本文口径。其中自己造成的红是两条——`453` 首跑被组内 `>` 截断、`461` 首跑缺 no-op 判定即抛；`459`（B4 点名 3.10 过期）与 `464`/`465`/`473`/`474`（唯一缺字节的那份就是审计器自己还没写完的日志）是设计红，按 `expected_exit_code=1` 记。`417`–`452` 已在册。
